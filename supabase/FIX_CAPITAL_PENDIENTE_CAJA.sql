-- =====================================================
-- FIX: Capital en caja sin doble conteo al pagar préstamos
-- =====================================================
-- Problema: restaba monto_prestado completo + sumaba pagos → al liquidar
--           (estado pagado) liberaba capital ya cobrado (+1000 artificial).
-- Solución: restar capital_pendiente (saldo de capital no recuperado).
--
-- Ejecutar en fases (producción activa):
--   A) supabase/FIX_CAPITAL_PENDIENTE_CAJA_A.sql  — solo funciones
--   B) supabase/FIX_CAPITAL_PENDIENTE_CAJA_B.sql  — prueba ANITA
--   C) supabase/FIX_CAPITAL_PENDIENTE_CAJA_C.sql  — resto de rutas (tras OK)
-- O: npx tsx scripts/apply-capital-pendiente-fix.ts A|B|C  (requiere DATABASE_URL)
...
BEGIN
  ...
  -- Préstamo abierto: saldo explícito
  ...
  -- Fallback amortización clásica (sin monto_capital en cuotas)
  RETURN GREATEST(
    0,
    v_monto_prestado
      - LEAST(
...
-- =====================================================
-- Suma capital pendiente en ruta (para UI / diagnóstico)
-- =====================================================
...
-- =====================================================
-- Recalcular capital con capital pendiente (no monto_prestado)
-- =====================================================
...
-- =====================================================
-- Trigger: abonos_capital → recalcular ruta
-- =====================================================
...
-- Verificación (no recalcula rutas — usar scripts A/B/C)
SELECT proname
FROM pg_proc
WHERE proname IN (
